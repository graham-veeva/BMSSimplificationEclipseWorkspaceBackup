    ...
    if (Trigger.isBefore) { 
        ...
        if ('false'.equalsIgnorecase (System.Label.DISABLE_VEEVA_MERGE_vod) && 
            (TSFList.size() > 0 || AddressList.size() > 0 || AffiliationList.size() > 0 
                || PriceList.size() > 0 || losingAtlWinningAccountList.size() > 0) || MCTargets.size() > 0 || Targets.size() > 0) {
            VEEVA_Merge.ProcessAccountMerge(TSFList, AddressList, AffiliationList, PriceList, losingAtlWinningAccountList, MCTargets, Targets, SetPrimaryAddr);
        }
        else
