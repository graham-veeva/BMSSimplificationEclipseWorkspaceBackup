trigger VOD_ACCOUNT_TO_TERRITORY_AFTER_TRIGGER on Account_Territory_Loader_vod__c (after insert,after update) {
     ...
     for (Integer i = 0 ;  i < Trigger.new.size(); i++)  {
        ...
        if(Trigger.isUpdate) {
          ...
          //  SystemSystem.debug('Old Territory List After processing    "' + Trigger.old[i].Territory_vod__c + '"');
          //  SystemSystem.debug('Old Territory to Drop After processing "' + Trigger.old[i].Territory_To_Drop_vod__c + '"');
        }
    ...
    if (VOD_ProcessATL.isAtlProcessed()) {
        return;
    }
